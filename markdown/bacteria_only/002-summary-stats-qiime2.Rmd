...
get_rarefied_table<-function(tax.df,tax.rank,host.classes){
  ...
  rare.table.fname <- file.path(community.composition.tables,
                                paste0(
                                  paste("ps.q.df.rare-nonfiltered",tax.rank,
                                        paste(host.classes,collapse = '-'),sep = "-"),
                                  ".tsv"))
  if(! file.exists (rare.table.fname)){
    write.table(tax.df.rare,
                file = rare.table.fname,
                row.names = F,
                sep = "\t")

  }

  rda.fname <- file.path(community.composition.rdafiles,paste0(
    paste("ps.q.df.rare-nonfiltered",tax.rank,
      paste(host.classes,collapse = '-'),sep = "-"),
    ".rds"))
...
for (image.format in c("png","tiff")){
  ggsave(file.path(community.composition.figures,
                paste0("Desulfobacterota-phylum-members-all-hosts.",
                      image.format)),
...
for (image.format in c("png","tiff")){
  ggsave(file.path(community.composition.figures,
                paste0("Spirochaetota-phylum-members-all-hosts.",
                      image.format)),
