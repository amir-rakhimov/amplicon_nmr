...
#'
phyloseq_mult_raref <- function(x, SampSize = NULL, MinSizeThreshold = NULL, 
  ...
  if(!is.null(MinSizeThreshold)){ x <- 
    phyloseq::prune_samples(phyloseq::sample_sums(x) >= MinSizeThreshold, x) }
  ...
  res <- plyr::mlply(
    .data = seeds,
    .fun = function(z, ...){ phyloseq::rarefy_even_depth(x, rngseed=z, 
