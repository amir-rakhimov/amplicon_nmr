...
metadata<-read.table(paste0(directory,"filenames-single-",authorname,"-raw-supercomp.tsv"),
                      header = T)

metadata<-metadata%>%
  filter(class=="NMR"|class=="B6mouse")
  # filter(class=="NMR")
...
# data(ko_abundance)
enz_abundance <- 
  read.table(paste0("./data/",authorname,
                    "-data/picrust/supercomp/NMR_B6mouse/pred_metagenome_unstrat.tsv"),
                            header = T,sep="\t")
...
# The columns specified after the exclamation mark are the ones 
# you want to exclude from the pivot operation.
enzymes.long<-enz_abundance%>%
  pivot_longer(!predicted.function,names_to = "Sample",values_to = "Abundance")

# split EC by level, each of the four digits goes into a separate column
enzymes.with.EC<-split.ec.levels(enzymes.long)

# merge with metadata, filter by EC 1 and EC 3
# unite will paste together EC levels into one EC group
...
# Add EC: in front of EC numbers
filtered.enzymes.long$predicted.function<-paste0("EC ",filtered.enzymes.long$predicted.function)
...
filtered.enz_abundance.nmr<-filtered.enz_abundance[which(rownames(filtered.enz_abundance)%in%
                               metadata$sample.id[which(metadata$class=="NMR")]),]
filtered.enz_abundance.B6<-filtered.enz_abundance[which(rownames(filtered.enz_abundance)%in%
                               metadata$sample.id[which(metadata$class=="B6mouse")]),]
enz.res_sim.nmr <- PERFect_sim(X = filtered.enz_abundance.nmr,alpha = 0.4)
enz.res_sim.B6 <- PERFect_sim(X = filtered.enz_abundance.B6,alpha = 0.4)
...
dim(enz.res_sim.nmr$filtX)      
colnames(enz.res_sim.nmr$filtX) # signal taxa

dim(enz.res_sim.B6$filtX)      
colnames(enz.res_sim.B6$filtX) # signal taxa
```
...
ecs<-enz.res_perm[["pvals"]][which(enz.res_perm[["pvals"]]<0.4)]
ecs.names<-sort(names(ecs))
kable(ecs.names,format = "simple")
ec.functions<-c(
 ...
 "EC 1.97.1" ="Other oxidoreductases",
 ...
 "EC 3.5.1" ="Acting on Carbon-Nitrogen Bonds, other than Peptide Bonds In Linear Amides",
 "EC 3.5.2"  ="Acting on Carbon-Nitrogen Bonds, other than Peptide Bonds In Cyclic Amides",
 "EC 3.5.99"  ="Acting on Carbon-Nitrogen Bonds, other than Peptide Bonds In Other Compounds"
  )
text.data<-data.frame(ec.functions)
text.data$predicted.function<-factor(rownames(text.data))
colnames(text.data)[1]<-"annotation"
# fast algo
set.seed(1)
enz.res_perm2 <- PERFect_perm(X = filtered.enz_abundance, Order = "pvals", pvals_sim = enz.res_sim, algorithm = "fast",alpha = 0.4)
...
```{r}
# we just take the long df and keep only those that match perfect output
enz.ec.df<-filtered.enzymes.long%>%
  group_by(predicted.function)%>%
  filter(predicted.function%in%ecs.names)
# we want to order by the median of boxplot, decreasing order
enz.ec.ordered<-enz.ec.df
predicted.function.ordered<-with(enz.ec.ordered,
                                 reorder(predicted.function,
                            Abundance,median,decreasing=F))
# make predicted function as factor
enz.ec.ordered$predicted.function<-factor(enz.ec.ordered$predicted.function,
                                          levels = levels(predicted.function.ordered))
text.data$predicted.function<-factor(text.data$predicted.function,
                                          levels = levels(predicted.function.ordered))
enz.ec.ordered%>%
  ggplot(aes(x=Abundance,y=predicted.function))+
  geom_boxplot()+
  theme_bw()
  
  # geom_text(data=text.data,
  #           aes(label=stringr::str_wrap(annotation),
  #               x=Inf),
  #           vjust = -0.5,
  #           hjust=1)+
...
ggsave(filename = "perfectplot.png",plot=p,
...
```{r}
# NMR with mice
enz.res_perm.all<- PERFect_perm(X = filtered.enz_abundance, 
                             Order = "pvals", 
                             pvals_sim = enz.res_sim, 
                             algorithm = "full",
                             alpha = 0.4,
                            )
ecs.all<-enz.res_perm.all[["pvals"]][which(enz.res_perm.all[["pvals"]]<0.25)]
ecs.all.names<-sort(names(ecs.all))
signif.all<-sort(ecs.all)
kable(ecs.all.names,format = "simple")
enz.all.df<-filtered.enzymes.long%>%
  group_by(predicted.function)%>%
  filter(predicted.function%in%ecs.all.names)%>%
  inner_join(metadata[,c("sample.id","class")],
             by=c("Sample"="sample.id"))%>%
  group_by(Sample)
# we want to order by the median of boxplot, decreasing order
enz.all.ordered<-enz.all.df
predicted.function.ordered.all<-with(enz.all.ordered,
                                 reorder(predicted.function,
                            Abundance,median,decreasing=F))
# make predicted function as factor
enz.all.ordered$predicted.function<-factor(enz.all.ordered$predicted.function,
                                          levels = levels(predicted.function.ordered.all))
text.data$predicted.function<-factor(text.data$predicted.function,
                                          levels = levels(predicted.function.ordered))
...
p.all<-enz.all.ordered%>%
  ggplot(aes(x=Abundance,y=predicted.function,fill=class))+
  geom_boxplot()+
  theme_bw()+
  scale_fill_manual(breaks=c("NMR","B6mouse"),
                    labels=c("Naked mole-rat","B6 mouse"),
                    values=custom.fill)+

  labs(fill = "Host",
       y="",
       x="PICRUSt2-predicted Abundance")+
  ggtitle("PICRUSt2-predicted Abundance of enzyme groups filtered by PERFect")+
...
ggsave(paste0("./images/taxaboxplots/",
              paste(Sys.Date(),"picrustboxplot",paste(custom.levels,collapse = '-'),
                    truncationlvl,
                    agglom.rank,"with-PERFect",sep = "-"),".tiff"),
       plot=p.all,
...
ggsave(paste0("./images/taxaboxplots/",
              paste(Sys.Date(),"picrustboxplot",paste(custom.levels,collapse = '-'),
                    truncationlvl,
                    agglom.rank,"with-PERFect",sep = "-"),".png"),
       plot=p.all,
       width = 4500,height = 3000,
...
```{r}
# full algo: NMR separate from mice
set.seed(1)
enz.res_perm.nmr<- PERFect_perm(X = filtered.enz_abundance.nmr, 
                             Order = "pvals", 
                             pvals_sim = enz.res_sim.nmr, 
                             algorithm = "full",
                             alpha = 0.4,
                            )
ecs.nmr<-enz.res_perm.nmr[["pvals"]][which(enz.res_perm.nmr[["pvals"]]<0.35)]
ecs.names.nmr<-names(sort(ecs.nmr))

set.seed(1)
enz.res_perm.B6<- PERFect_perm(X = filtered.enz_abundance.B6, 
                             Order = "pvals", 
                             pvals_sim = enz.res_sim.B6, 
                             algorithm = "full",
                             alpha = 0.4,
                            )
ecs.B6<-enz.res_perm.B6[["pvals"]][which(enz.res_perm.B6[["pvals"]]<0.35)]
ecs.names.B6<-names(sort(ecs.B6))

ecs.all.names<-c(ecs.names.nmr,ecs.names.B6)
enz.all.df<-filtered.enzymes.long%>%
  group_by(predicted.function)%>%
  filter(predicted.function%in%ecs.all.names[1:9])%>%
  inner_join(metadata[,c("sample.id","class")],
             by=c("Sample"="sample.id"))%>%
  group_by(Sample)
# we want to order by the median of boxplot, decreasing order
...
p.all<-enz.all.ordered%>%
  ...
  ggplot(aes(x=log10Abundance,y=predicted.function,fill=class))+
  ...
  labs(fill = "Host",
       y="",
       x="PICRUSt2-predicted Abundance (log10)")+
  ggtitle("PICRUSt2-predicted Abundance of enzyme groups filtered by PERFect")+
  theme(legend.position = c(.9,0.1),
        axis.line = element_blank(), 
        strip.text.y = ggtext::element_markdown(size = 20),
        panel.spacing = unit(0.8, "cm"), # increase distance between facets
