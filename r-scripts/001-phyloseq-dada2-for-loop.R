...
for (truncationlvl in trunclvls){
  ...
  ps<-qza_to_phyloseq(
    ...
    features = paste0(qiimedir,"yashliu","-table-trimmed-dada2-",
                      truncationlvl,".qza"),
    taxonomy = paste0(qiimedir,"yashliu","-taxonomy-trimmed-dada2-",
                      truncationlvl,".qza"),
    tree = paste0(qiimedir,"yashliu","-rooted-tree-trimmed-dada2-",
                  truncationlvl,".qza")
  )
  # add custom metadata cause previous command loses a sample for some reason
  custom.md<-read.table(paste0(qiimedir,"filenames-",authorname,"-final-supercomp.tsv"),
                       header = T)
  ...
  head(ps.agg.rel) 
  ps.agg.rel$Sample<-paste(ps.agg.rel$Sample,"final",sep="_")
  ...
  ps.total$Sample<-paste(ps.total$Sample,"final",sep="_")
